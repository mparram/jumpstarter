"""
HTTP server to expose metrics in Prometheus format.
"""
...
class MetricsServer:
    ...
    async def start(self):
        ...
        if not self.config.enabled:
            logger.debug("Metrics server is disabled, not starting")
            return
        ...
        logger.info("Metrics server started on http://%s:%d/metrics", self.config.host, self.config.port)

    async def stop(self):
        """Stop the HTTP metrics server."""
        if not self.config.enabled or self.runner is None:
            return
